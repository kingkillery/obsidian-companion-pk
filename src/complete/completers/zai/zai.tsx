...
export default class ZAIModel implements Model {
	id: string;
	name: string;
	description: string;
	...
	async complete(prompt: Prompt, settings: string): Promise<string> {
		...
		if (response.status === 429) {
			new Notice("Z.AI rate limit exceeded. Please wait.");
			throw new Error("Rate limit");
		}
		if (response.status === 401) {
			new Notice("Z.AI API key is invalid.");
...
export class ZAIComplete implements Completer {
	id: string = "zai";
	name: string = "Z.AI";
	description: string = "Z.AI API (Anthropic-compatible)";
