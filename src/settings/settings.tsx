...
function AcceptSettingsComponent({
	...
	return (
		<>
			...
			<SettingsItem
				name="Slash command mode"
				description="Enable the slash command palette and context-aware actions."
			>
				<div
					className={
						"checkbox-container" +
						(plugin.settings.slash_commands_enabled ? " is-enabled" : "")
					}
					onClick={(_e) => {
						plugin.settings.slash_commands_enabled =
							!plugin.settings.slash_commands_enabled;
						plugin.saveData(plugin.settings);
						reload_signal.reload = true;
					}}
			...
			{plugin.settings.slash_commands_enabled ? (
				<>
					<SettingsItem
						...
						description="Maximum slash suggestions shown."
					>
						<input
							type="number"
							value={plugin.settings.slash_max_suggestions}
							onChange={(e) => {
								plugin.settings.slash_max_suggestions = parseInt(
									e.target.value
								);
								plugin.saveData(plugin.settings);
							}}
					...
					<SettingsItem
						name="Link score threshold"
						description="Minimum confidence for link intelligence suggestions."
					>
						<input
							type="number"
							step="0.01"
							min="0"
							max="1"
							value={plugin.settings.link_intelligence_min_score}
							onChange={(e) => {
								plugin.settings.link_intelligence_min_score = parseFloat(
									e.target.value
								);
								plugin.saveData(plugin.settings);
							}}
					...
					<SettingsItem
						...
						description="Folder path for daily notes."
					>
						<input
							type="text"
							value={plugin.settings.daily_note_path}
							onChange={(e) => {
								plugin.settings.daily_note_path = e.target.value;
								plugin.saveData(plugin.settings);
							}}
						/>
					</SettingsItem>
