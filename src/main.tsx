...
interface CompanionSettings {
	...
	slash_max_suggestions: number;
	link_intelligence_min_score: number;
...
const DEFAULT_SETTINGS: CompanionSettings = {
	...
	accept: {
		...
		completion_completeness_regex: ".*[^a-zA-Z]$",
		min_accept_length: 4,
	...
	slash_max_suggestions: 6,
	link_intelligence_min_score: 0.72,
...
export default class Companion extends Plugin {
	...
	async setupSlashCommands() {
		this.link_graph_service = new LinkGraphService(this.app);
		await this.link_graph_service.bootstrap();
		this.slash_command_service = new SlashCommandService(this);
		if (this.settings.slash_commands_enabled) {
			this.addCommand({
				id: "open-slash-command-palette",
				name: "Open slash command palette",
				editorCallback: (editor: Editor) => {
					const view = this.app.workspace.getActiveViewOfType(MarkdownView);
					if (!view) return;
					const context = this.extractSlashContext(editor, view);
					if (!context) {
						return;
					}
					void this.openSlashPalette(editor, view, context);
				},
			});

			this.registerEvent(
				this.app.workspace.on("editor-change", (editor: Editor) => {
					if (!this.settings.slash_commands_enabled) return;
			...
			this.registerEvent(
				this.app.metadataCache.on("changed", (file: any) => {
					if (file && file.path && this.link_graph_service) {
						const existing = this.app.vault.getAbstractFileByPath(file.path);
						if (existing instanceof TFile) {
							this.link_graph_service.refresh_file(existing);
						}
					}
				})
			);
			this.registerEvent(
				this.app.vault.on("modify", (file: TFile) => {
					if (this.link_graph_service) {
						this.link_graph_service.refresh_file(file);
					}
				})
			);
			this.registerEvent(
				this.app.vault.on("delete", (file: TFile) => {
					if (this.link_graph_service) {
						this.link_graph_service.remove_file(file.path);
					}
				})
			);
			this.registerEvent(
				this.app.vault.on("rename", (file: TFile, old_path: string) => {
					if (this.link_graph_service) {
						this.link_graph_service.rename_file(old_path, file.path);
					}
				})
			);
		}
	...
	private extractSlashContext(
		editor: Editor,
		view: MarkdownView
	): SlashContext | null {
		if (!this.link_graph_service || !this.slash_command_service) {
			return null;
		}
		...
		const candidate =
			candidates.find((entry) => {
				const before_trigger = entry.index - 1;
				return entry.index === 0 || /\s/.test(before_cursor[before_trigger]);
			}) || null;
		if (!candidate) return null;

		const token_index = candidate.index;
		const token = before_cursor.substring(token_index);
		if (!token.startsWith(candidate.trigger)) {
			return null;
		}
		...
		return {
			...
			view,
			plugin: this,
			command_trigger: candidate.trigger,
			file,
			cursor,
			line_text: line,
	...
	private async openSlashPalette(
		editor: Editor,
		view: MarkdownView,
		context: SlashContext
	) {
		if (!this.settings.slash_commands_enabled || !this.slash_command_service) {
			return;
		}

		if (!this.slash_command_modal) {
			this.slash_command_modal = new SlashCommandModal(
				this.app,
				context.query,
				context.command_trigger,
				(query) => {
					const latest_context = this.extractSlashContext(editor, view);
					return this.slash_command_service!.get_suggestions(
						query,
						latest_context || context,
						(latest_context || context).command_trigger
					);
				},
				async (suggestion) => {
			...
			this.slash_command_modal.open();
		} else {
			this.slash_command_modal.setQuery(context.query);
			this.slash_command_modal.setTrigger(context.command_trigger);
		}
	}
	...
	async generate_text_snippet(
		prompt: string,
		max_chars: number = 1200
	): Promise<string> {
		let output = "";
		const model_completion = this._complete(
			prompt,
			"",
			this.settings.provider,
			this.settings.model
		);
		for await (const completion of model_completion) {
			output += completion.complete_suggestion || completion.display_suggestion || "";
